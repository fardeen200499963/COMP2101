...
# FARDEEN FARDEEN FARDEEN FARDEEN FARDEEN FARDEEN FARDEEN
thisInterfaceRequested=""
...
while [ $# -gt 0 ]; do
    ...
    -v)
        ...
        ;;
    *)
        if [ "$thisInterfaceRequested" == "" ]; then
            thisInterfaceRequested="$1"
        fi
        ;;
    esac
    shift
...
# interface="eno1"
if [ "$thisInterfaceRequested" != "" ]; then
    interface=$thisInterfaceRequested
...
allInterface=$(nmcli device status | awk '{print $1}' | awk '(NR>1)' | head -n -1)
